...
processing_duration%>%
  ggplot(
    aes(x = factor(var, level = c("constructing_implications_duration", "analyzing_errors_duration", "comparing_perspective_duration", "noticing_results_duration")), 
    y = val, fill = var)) + 
...
```{r}
  ...
  geom_segment(data = eventdata_processing_sequences %>%
...
ggsave("Figure 3_Sequences of main steps per teacher_portrait.png", plot = last_plot(), device = "png", units = c("cm"), width = 34, height = 48, dpi = 300)
...
```{r}
 ...
 facet_wrap(~`pid`, ncol = 4) + 
  ...
  ylab("") +
  theme_minimal() + 
   theme(legend.position = "bottom", legend.justification = "center", legend.direction = "horizontal", legend.title = element_text(size = 16), 
...
ggsave("Figure 3_Sequences of main steps per teacher_landscape.png", plot = last_plot(), device = "png", units = c("cm"), width = 80, height = 48, dpi = 300)
...
dep_matrix_05 <- dependency_matrix(eventlog, 
                                  dependency_type =  dependency_type_fhm(threshold_dependency = 0.5, 
                                  threshold_l1 = 0, # loop 1
                                  threshold_l2 = 0, # loop 2
                                  threshold_frequency = 0, # frequency
                                  all_connected = TRUE, # T = connecting best antecedent and consequent regardless of threshold
